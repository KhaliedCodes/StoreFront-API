...
userRouter.get("/show",authenticate, query("id").isNumeric(), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await context.show(parseInt(req.query.id as string));
    if(user)
...
userRouter.post("/create",body('username').isLength({min:5, max:20}), body('password').isLength({min:8}), async (req: Request, res: Response) => {
  const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({errors: errors.array()});
    }
